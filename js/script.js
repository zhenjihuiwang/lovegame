...
const parallaxManager = {
    ...
    toggle: function() {
        ...
        localStorage.setItem('enable_parallax', this.enabled);
    ...
    handleOrientation: function(e) {
        ...
        const x = e.gamma; // -90 to 90 (左右倾斜)
        const y = e.beta;  // -180 to 180 (前后倾斜)
        ...
        document.querySelectorAll('.parallax-layer').forEach(el => {
            ...
            const moveX = clampedX * depth * 20; 
            const moveY = clampedY * depth * 20;
    ...
    init: function() {
        ...
        document.getElementById('gyro-toggle').checked = saved;
        if(saved) this.toggle(); // 自动开启
    }
};
...
const characterManager = {
    ...
    init: function() {
        ...
        if (savedList) {
            ...
            if (oldName) {
                const newChar = {
                    id: Date.now().toString(), name: oldName,
                    prompt: localStorage.getItem('conf_sysPrompt') || "",
                    userName: localStorage.getItem('conf_userName') || "玩家",
                    userDesc: localStorage.getItem('conf_userDesc') || "", relation: localStorage.getItem('conf_userRelation') || ""
                };
    ...
    createNew: function(silent = false) {
        const newChar = {
            id: Date.now().toString(), name: "新角色", prompt: "在此输入人设...",
            userName: "玩家", userDesc: "", relation: "初识"
        };
        this.list.push(newChar); this.currentId = newChar.id;
        this.save(); this.renderList(); this.loadCurrent();
        assetManager.refreshCache(); 
        if(!silent) uiManager.switchTab('persona');
    },
    select: function(id) {
        this.currentId = id; this.save();
        this.renderList(); this.loadCurrent();
    ...
    deleteCurrent: function() {
        ...
        if(confirm(`确定删除 ${this.getCurrent().name}?`)) {
            const idx = this.list.findIndex(c => c.id === this.currentId);
            ...
            this.save(); this.select(this.currentId);
        }
    },
...
const timeManager = {
    getTimeContext: function() {
        ...
        else if (hour >= 9 && hour < 17) timeOfDay = "白天";
        else if (hour >= 17 && hour < 23) timeOfDay = "晚上";
        else timeOfDay = "深夜";
        ...
        if (lastTime) {
            ...
            if (diffHrs < 1) intervalDesc = "刚刚";
            else if (diffHrs < 24) intervalDesc = "距上次见面不到一天";
            else intervalDesc = `好久不见 (${Math.floor(diffHrs/24)} 天没聊了)`;
        }
...
const aiEngine = {
    ...
    triggerGreeting: function() {
        ...
        [事件: APP启动]
        时间: ${timeCtx.fullTime} (${timeCtx.timeOfDay}). 间隔: ${timeCtx.interval}
        节日: ${timeCtx.festival}.
        任务: 请根据时间和人设生成一段开场剧情。
        `;
    ...
    buildSystemPrompt: function() {
        ...
        2. **表演克制**：默认只使用 Level 1 情绪。严禁在没有铺垫的情况下突然切换到 Level 2。只有当对话内容极其悲伤或极其快乐时，才允许使用 Level 2。
